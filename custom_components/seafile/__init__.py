...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    _updater: SeafileUpdater = SeafileUpdater(
        hass,
        get_config_value(entry, CONF_URL),
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if is_unload := await hass.config_entries.async_unload_platforms(entry, PLATFORMS):
        _updater: SeafileUpdater = hass.data[DOMAIN][entry.entry_id][UPDATER]
        await _updater.async_stop()
